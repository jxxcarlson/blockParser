...
In addition to the code in `./tests` and `./benchmarks`,
there is the folder `./tools`.  It provides a command-line tool 
for experimenting with the the parser. It is currently configured
to parse text in `LanguageB` and display a simplified version of
the parse tree, displaying the source and depth of the block
labeling each node. This tools is easily configurable.  Simply
change the `transform` function in `./tools/src/Main.elm`
